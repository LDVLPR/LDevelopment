...
namespace LDevelopment.Interfaces
{
    ...
    {
        T Find<T>(int id, params Expression<Func<T, object>>[] items) where T : class, IModel;

        T Find<T>(Expression<Func<T, bool>> predicate, params Expression<Func<T, object>>[] items) where T : class, IModel;
